    ...
    return (
        <ul role="list" className="divide-y divide-gray-100">
            ...
            {listOrders.map((order) => (
                ...
                <li key={order.id} className="flex flex-col items-center md:flex-row justify-around">
                    <div>
                        <img className="h-40 md:h-20 w-auto flex rounded-full bg-gray-50" src={`${assetsPath}/order.png`} alt="" />
                    </div>
                    <div className="flex text-darkOrange flex-col md:order-last hover:opacity-70 cursor-pointer" onClick={() => handleDeleteOrder(order.id)}>
                        <button
                            className=" text-4xl md:text-xl p-1 rounded-xl "
                        >
                            <BsFillTrash2Fill />
                        </button><span className="md:hidden">Delete</span></div>
                    <div className="flex flex-col items-end md:flex-row md:gap-10">
                        <div> <p className="py-2 mt-2 truncate text-sm leading-5 text-gray-500"><span className="font-bold pr-2">OrderId:</span>{order.id.slice(-6)}</p>
                            ...
                            <Menu as="div" className="relative ml-3">
                                <div>
                                    <Menu.Button className="relative flex items-center rounded-full text-sm focus:outline-none focus:ring-2 focus:ring-white focus:ring-offset-2 focus:ring-offset-gray-800">
                                        ...
                                        <span className="sr-only">Open listo of products menu</span>
                                        <img
                            ...
                            <p className="px-5 py-2 mt-5 truncate text-sm leading-5 text-gray-500"><span className="font-bold">Total:</span>COP/{order.total}</p></div>
                        <div>                    
                            <p className="py-2 mt-2 text-sm leading-5 text-gray-500"><span className="font-bold mr-2">Date:</span>{new Date(order.date).toDateString()}</p>
                            <Menu as="div" className="relative ml-3">
                                <div>
                                    <Menu.Button className="relative flex items-center rounded-full text-sm focus:outline-none focus:ring-2 focus:ring-white focus:ring-offset-2 focus:ring-offset-gray-800">
                                        <span className="absolute -inset-1.5" />
